 ...
 *
 * Class NSOF_Field_Container_Base
 * @since  1.0.0
 */
abstract class NSOF_Field_Container_Base extends NSOF_Field_Base {
	/**
	...
	protected function render_before_field( $value, $instance ) {
		if( ! empty( $value[ 'field_container_state' ] ) ) {
			$this->state = $value[ 'field_container_state' ];
		}
		else {
	...
	protected function get_label_classes( $value, $instance ) {
		...
		if( $this->state == 'open' ) $label_classes[] = 'nsof-section-visible';
	...
	protected function create_and_render_sub_fields( $values, $parent_container = null, $is_template = false ) {
		...
		/* @var $field_factory NSOF_Field_Factory */
		$field_factory = NSOF_Field_Factory::getInstance();
		foreach( $this->fields as $sub_field_name => $sub_field_options ) {
			/* @var $field NSOF_Field_Base */
