...
(function( $ ) {
    var setup = function(widgetForm) {
        if(typeof tinyMCEPreInit !== 'undefined') {
            ...
            $(widgetForm).find('> .nsof-field-type-tinymce > .nsof-tinymce-container').each(function (index, element) {
                ...
                qtSettings = $.extend({}, tinyMCEPreInit.qtInit['nsof-input-tinymce-field'], qtSettings, {id: id});
                ...
                $(this).on( 'click', function(event) {
                    ...
                    if ( $target.hasClass( 'wp-switch-editor' ) ) {
                        ...
                        $(this).find('+ .nsof-tinymce-selected-editor').val(mode);
                    }
                });
            });
            QTags._buttonsInit();
    ...
    $(document).on( 'nsofsetupform', function(e) {
        var $f = $(e.target);

        if($f.is('.nsof-field-repeater-item-form')){
            if($f.is(':visible')) {
                setup( $f );
            }
            else {
    ...
    $(document).on('sortstop', function (event, ui) {
        if(ui.item.is('.nsof-field-repeater-item')) {
            ui.item.find('> .nsof-field-repeater-item-form').each(function(){
                setup( $(this) );
            });
        }
        else {
            setup(ui.item.find('.nsof-form'));
        }
    });
