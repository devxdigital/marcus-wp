var soWidgetPostSelector = ( function ($, _) {
    ...
    PostCollection = self.PostCollection = Backbone.Collection.extend( {
        ...
        updateWithQuery: function(query){
            ...
            var c = this;
            $.post(
                nsofPostsSelectorTpl.ajaxurl,
                { action: 'nsof_get_posts', query: query, 'ignore_pagination' : true },
                function(data){
    ...
    QueryBuilder = self.QueryBuilder = Backbone.View.extend( {
        ...
        render: function() {
            // Create the modal
            this.$el.html( nsofPostsSelectorTpl.modal );
    ...
    QueryForm = self.QueryForm = Backbone.View.extend( {
        ...
        render: function(){
            ...
            // The post type field
            this.form.append('<div class="query-builder-form-field">' + nsofPostsSelectorTpl.fields.post_type + '</div>');
            if( typeof this.model.get('post_type') !== 'undefined' ) this.form.find('select[name="post_type"]').val( this.model.get('post_type'));

            // The post__in field
            this.form.append('<div class="query-builder-form-field">' + nsofPostsSelectorTpl.fields.post__in + '</div>');
            if( typeof this.model.get('post__in') !== 'undefined' ) this.form.find('input[name="post__in"]').val( this.model.get('post__in').join(',') );

            // The taxonomy field
            this.form.append('<div class="query-builder-form-field ui-front">' + nsofPostsSelectorTpl.fields.tax_query + '</div>');
            if( typeof this.model.get('tax_query') !== 'undefined' ) this.form.find('input[name="tax_query"]').val( this.model.get('tax_query'));

            // The order field
            this.form.append($('<div class="query-builder-form-field">' + nsofPostsSelectorTpl.fields.orderby + '</div>').disableSelection());
            ...
            this.form.append('<div class="query-builder-form-field">' + nsofPostsSelectorTpl.fields.posts_per_page + '</div>');
            ...
            this.form.append('<div class="query-builder-form-field">' + nsofPostsSelectorTpl.fields.sticky + '</div>');
            ...
            this.form.append('<div class="query-builder-form-field">' + nsofPostsSelectorTpl.fields.additional + '</div>');
            ...
            var orderButton = orderField.closest('.query-builder-form-field').find('.nsof-order-button');

            // Reset the ordering button
            var resetOrderButton = function () {
                if (orderField.val() === 'DESC') {
                    orderButton.removeClass('nsof-order-button-asc');
                    orderButton.addClass('nsof-order-button-desc');
                }
                else {
                    orderButton.addClass('nsof-order-button-asc');
                    orderButton.removeClass('nsof-order-button-desc');
                }
            };
            resetOrderButton();
            ...
            this.$el.find('.query-builder-form-field .nsof-select-posts').click(function(e){
                e.preventDefault();
    ...
    PostCollectionSummaryView = self.PostCollectionSummaryView = Backbone.View.extend( {
        template: _.template(nsofPostsSelectorTpl.foundPosts),
        posts: null,
    ...
    PostCollectionView = self.PostCollectionView = Backbone.View.extend( {
        ...
        template: _.template(nsofPostsSelectorTpl.postSummary),
        ...
        render: function(){
            var $c = this.$el.find('.query-builder-content').empty().append('<div class="nsof-current-posts"></div>').find('.nsof-current-posts');
    ...
    PostSelectView = self.PostSelectView = Backbone.View.extend( {
        ...
        postCache: {},
        postTemplate: _.template(nsofPostsSelectorTpl.postSummary),
        ...
        render: function(){
            ...
            this.$el.find('.query-builder-content').empty().html(nsofPostsSelectorTpl.selector);

            // Set up the sortable
            this.sortable = this.$el.find('.query-builder-content #nsof-post-selector .nsof-posts-sortable').sortable({
                placeholder: "ui-state-highlight",
                forcePlaceholderSize: true,
                items : '> .nsof-post-selector-summary'
            });
            ...
            var v = this;
            var $searchInput = this.$el.find('.query-builder-content #nsof-post-selector .nsof-search-field');

            $searchInput.autocomplete({
                source: function(request, response) {
                    request.type = postType;
                    request.action = 'nsof_selector_search_posts';
                    $.get(
                        nsofPostsSelectorTpl.ajaxurl,
                        request,
                        response
            ...
            this.$el.find('.query-builder-content').on('click', '.nsof-remove', function(e){
                e.preventDefault();
                var $$ = $(this);
                $$.closest('.nsof-post-selector-summary').fadeOut('fast', function(){
                    $(this).remove();
        ...
        addPosts: function(posts) {
            ...
            for(var i = 0; i < posts.length; i++) {
                if( typeof this.postCache[posts[i]] === 'undefined' ) {
                    ...
                    postItem = $(this.postTemplate( {id:posts[i], title: '', thumbnail: '', editUrl: '#'} )).addClass('nsof-post-loading');
                }
                else {
        ...
        refreshLoading: function(){
            var v = this;
            this.sortable.find('.nsof-post-selector-summary.nsof-post-loading').each(function(){
                var $$ = $(this);
                var id = $$.data('id');

                if(typeof v.postCache[id] !== 'undefined') {
                    $$.removeClass('nsof-post-loading');
        ...
        buttonHandler: function(){
            ...
            var ids = [];
            this.sortable.find('.nsof-post-selector-summary').each(function(){
    ...
    jQuery( function($){
        $('body').on('click', '.nsof-select-posts', function(e){
            e.preventDefault();
            var $postSelectorButton = $(this);
            builder.model.setSyncField( $postSelectorButton.siblings( '.nsof-input' ) );
            builder.model.sync('update');
            builder.views.postSummary.posts.on("reset", function (postsCollection) {
                $postSelectorButton.find(".nsof-current-count").text(postsCollection.foundPosts);
            });
            builder.open();
        });
    } );
