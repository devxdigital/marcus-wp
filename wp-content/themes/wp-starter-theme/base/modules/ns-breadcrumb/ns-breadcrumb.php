...
class NS_breadcrumb{
	...
	private function set_opt($opt=''){
		...
		$el_prefix = '<li><a href="'.home_url('/').'">Home</a></li>';
		$blog_prefix = '<li><a href="'.get_post_type_archive_link( 'post' ).'">'.get_the_title( get_option('page_for_posts', true) ).'</a></li>';
		...
		$set_opt = apply_filters('bdb_set_opt', array(
			...
			'author'      => 'Archived Article(s) by Author: ',
	...
	public function output_html(){
		...
		if (!is_front_page()) {
			...
			if (is_single() && in_array('single', $templates)) {
				$output = $this->set_opt('blog_prefix');
				$output .= $this->is_single_post();
			}
			//breadcrumb for blog page
			elseif(is_home() && in_array('blog', $templates)){
				$output .= '<li>'.get_the_title( get_option('page_for_posts', true) ).'</li>';
			}
			//breadcrumb for category and sub-category archive
			elseif(is_category() && in_array('category', $templates)){
				$output = $this->set_opt('blog_prefix');
				$output .= '<li>'.$this->set_opt('category') . get_category_parents($cat, false, '').'</li>';
			}
		...
		if(!empty($output)){
			...
			$return = '<nav class="breadcrumbs">';
				$return .= '<ul class="breadcrumbs__menu horizontal-list horizontal-list-alt">';
