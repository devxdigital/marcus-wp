...
function if_bootstrap_widgets( $widget_output, $widget_type, $widget_id, $sidebar_id ) {
	switch( $widget_type ) {
		case 'categories' :
            $widget_output = str_replace('<ul>', '<ul class="list-group list-group-flush left-menu"><li class="list-group-item d-flex align-items-center cat-item cat-item-0'.(is_home() ? ' current-cat' : '').'"><a aria-current="page" href="'.get_post_type_archive_link( 'post' ).'">All</a> ('.wp_count_posts()->publish.')</li>', $widget_output);
            $widget_output = str_replace('<li class="cat-item cat-item-', '<li class="list-group-item d-flex align-items-center cat-item cat-item-', $widget_output);
...
function trigger_custom_scripts() {
    ...
    wp_register_script('jquery', get_template_directory_uri() . '/assets/js/jquery-3.3.1.min.js', array(), '3.4.1');
