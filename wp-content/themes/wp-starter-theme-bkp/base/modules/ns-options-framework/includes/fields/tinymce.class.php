...
/**
 * Class NSOF_Field_TinyMCE
 * @since  1.0.0
 */
class NSOF_Field_TinyMCE extends NSOF_Field_Text_Input_Base {
	/**
	...
	function __call( $name, $arguments ) {
		if ( preg_match( '/mce_buttons(?:_[1-4])?|quicktags_settings/', $name ) && !empty( $this->button_filters[$name] ) ) {
			...
			if ( !empty( $filter[0] ) && is_a( $filter[0], 'NS_OF' ) ) {
				$widget = $filter[0];
	...
	protected function render_field( $value, $instance ) {
		...
		?>
		<div class="nsof-tinymce-container"
		...
		<input type="hidden"
			   name="<?php echo esc_attr( substr($this->element_name, 0, -1) . '_selected_editor]'); ?>"
			   class="nsof-input nsof-tinymce-selected-editor"
	...
	public function enqueue_scripts() {
		wp_enqueue_script( 'nsof-tinymce-field', NSOF_URI.'/assets/js/tinymce-field.js', array( 'jquery', 'editor', 'quicktags' ) );
		wp_enqueue_style( 'nsof-tinymce-field', NSOF_URI.'/assets/css/tinymce-field.css', array() );
	}
