 ...
 *
 * Class NSOF_Field_Media
 * @since  1.0.0
 */
class NSOF_Field_Media extends NSOF_Field_Base {
	/**
	...
	protected function get_default_options() {
		return array(
			'choose' => __( 'Choose Media', 'wp-starter-theme' ),
			'update' => __( 'Set Media', 'wp-starter-theme' ),
			'library' => 'image'
		);
	}
	...
	protected function render_field( $value, $instance ) {
		if( version_compare( get_bloginfo('version'), '3.5', '<' ) ){
			printf( __('You need to <a href="%s">upgrade</a> to WordPress 3.5 to use media fields', 'wp-starter-theme'), admin_url('update-core.php') );
			return;
		}
		...
		<div class="media-field-wrapper">
			<div class="current">
				<div class="thumbnail-wrapper">
					<img src="<?php echo nsof_esc_url( $src[0] ) ?>" class="thumbnail" <?php if( empty( $src[0] ) ) echo "style='display:none'" ?> />
				</div>
		...
		<a href="#" class="media-remove-button <?php if( empty( $value ) ) echo 'remove-hide'; ?>"><?php esc_html_e( 'Remove', 'wp-starter-theme' ) ?></a>

		<input type="hidden" value="<?php echo esc_attr( is_array( $value ) ? '-1' : $value ) ?>" name="<?php echo esc_attr( $this->element_name ) ?>" class="nsof-input" />
	...
	protected function render_after_field( $value, $instance ) {
		if( !empty( $this->fallback ) ) {
			...
			<input type="text" value="<?php echo esc_url( $fallback_url ) ?>"
			    placeholder="<?php esc_attr_e( 'External URL', 'wp-starter-theme' ) ?>"
			    name="<?php echo esc_attr( $this->element_name . '_fallback') ?>"
			    class="media-fallback-external nsof-input" />
			<div class="clear"></div>
			<?php
		}
		else {
	...
	public function sanitize_instance( $instance ) {
		...
		if( !empty( $this->fallback ) && !empty( $instance[ $fallback_name ] ) ) {
			$instance[ $fallback_name ] = nsof_esc_url_raw( $instance[ $fallback_name ] );
		}
		return $instance;
	}
